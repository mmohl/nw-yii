...
$(document).on("click", "#table-customer-list tbody tr", ({ target }) => {
  ...
  const status = $(tdStatus).children("span").html();
...
$("#input-payment").on(
  "keyup",
  _.debounce(({ target: { value } }) => {
    const inputPay = parseInt(value.replace(/\./gi, ""));
    const totalShouldPay = parseInt(
      $("#input-total").val().replace(/\./gi, "")
    );
...
$("#btn-invoice-pay").on("click", () => {
  ...
  const payment = $("#input-payment").val()
    ? $("#input-payment").val().replace(/\./gi, "")
    : null;
...
function renderTotalItem(res, status, isIgnored = false) {
  resetViewInvoice();
  if (res.is_ignored == 1) isIgnored = true;
  ...
  if (isIgnored) {
    ...
    $("#input-total").val(
      (res.total - res.taxTotal + res.rounded).toLocaleString("id")
    );
  } else {
  ...
  if (status.toLowerCase() == "lunas") {
    ...
    $("#input-changes").val(
      (res.total_payment - (res.total + res.rounded)).toLocaleString("id")
    );
    // $('#input-ignored').prop('disabled', true)
    if (res.is_ignored == "1") {
      //   $("#input-ignored").prop("checked", true);
      //   $("#input-ignored").prop("disabled", true);
    }
  } else {
    $("#input-changes").val("");
...
function init() {
  dt = $("#table-customer-list").DataTable({
    ...
    initComplete: function () {
      $("div.toolbar").prepend(
        `<button type="button" onclick="resetViewInvoice()" style="position: absolute;" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-erase"></span> Hapus Detail</button>`
      );
    ...
    columns: [
      ...
      { data: "table_number", sortable: false },
      {
        ...
        render: (data, i, row) =>
          `Rp ${parseInt(
            data.reduce((p, n) => (p += parseInt(n.qty) * parseInt(n.price)), 0)
          ).toLocaleString("id")}`,
      },
      {
        data: "is_paid",
        ...
        sortable: false,
        render: (data, i, row) =>
          `<span class="badge badge-${data == 1 ? "primary" : "danger"}">${
            data == 1 ? "Lunas" : "Belum Bayar"
          }</span>`,
      },
    ],
